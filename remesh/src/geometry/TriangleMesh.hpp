...
{
    boost::unordered_map< int, boost::unordered_map<int, int> > hash;
    int v0, v1;
...
void TriangleMesh<T>::bounding_box(Point3<T>& low, Point3<T>& up) const
{
    ...
    for(typename std::vector< Point3<T> >::const_iterator it = vertices_.begin();
            it != end;++ it)
    {
        low.x = std::min(low.x, it->x);
        low.y = std::min(low.y, it->y);
        low.z = std::min(low.z, it->z);

        up.x = std::max(up.x, it->x);
        up.y = std::max(up.y, it->y);
        up.z = std::max(up.z, it->z);
    }
}
