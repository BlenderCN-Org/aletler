...
public:
	...
	double mass_air = volume * RHO_AIR;
	double mass_water = volume * RHO_WATER;
	_a = GRAVITY * (mass_water - mass_air) / (mass_air + mass_water);
	_v = 0;
    ...
    double minnaert(double mmradius) {
        ...
        double p0 = P_ATM;
        return (0.5 / M_PI) * (1.0 / m_radius) * sqrt(3 * GAMMA_AIR * p0 / RHO_WATER);
    ...
    double pressure_hydrostatic(double depth) {
      return P_ATM + RHO_WATER * GRAVITY * depth;
    }

    double pressure_surfacetension() {
      return 2 * SIGMA_AIR_WATER / m_radius;
    }
    ...
    double frequency() {
      ...
      double numer = 4 * M_PI * GAMMA_AIR * P0 * C;
      double denom = RHO_WATER * V0;
